...
export function computeBookingQuoteBreakdown(state: BookingState): BookingQuoteBreakdown | null {
...
export function computeBookingPricing(state: BookingState): BookingPricing | null {
  if (!state.serviceType || !state.jobType) return null
  if (
    (state.jobType === 'deliveryPickup' ||
      state.jobType === 'heavyItem' ||
  ...
  const totalItems =
    Object.values(state.itemCounts ?? {}).reduce((sum, qty) => sum + Math.max(0, qty || 0), 0) ||
    state.detectedItems.length
  const breakdown = computeBookingQuoteBreakdown(state)
  if (!breakdown) return null

  const junkTier =
    state.serviceType === 'remove'
      ? totalItems >= 15
        ? 'full truck'
        : totalItems >= 8
          ? 'half truck'
          : totalItems >= 3
            ...
            : 'single item'
      : null
  const explanationParts = state.serviceType === 'remove'
    ? [
        'junk removal',
        junkTier ?? '',
        `${totalItems} item${totalItems === 1 ? '' : 's'}`,
        state.disposalRequired ? 'incl. disposal' : '',
      ].filter(Boolean)
    : [
  ...
  if (state.serviceType === 'helpers') {
    explanationParts.splice(
      ...
      'helpers job',
      `${Math.max(1, state.helperHours ?? 1)} hr`,
    )
  ...
  if (state.serviceType === 'cleaning') {
    explanationParts.splice(
      ...
      explanationParts.length,
      'cleaning job',
      `${Math.max(1, state.cleaningHours ?? 1)} hr`,
    )
  ...
  if (state.serviceType !== 'helpers' && state.serviceType !== 'cleaning' && breakdown.autoHelpers > 0) {
    explanationParts.push(
      `${breakdown.autoHelpers} helper${breakdown.autoHelpers > 1 ? 's' : ''}`,
    )
  }
...
/**
 * Backward-compatible pricing helper for the new booking state shape.
 */
export function computeBookingPriceRange(state: BookingState): BookingPriceRange | null {
  const pricing = state.pricing ?? computeBookingPricing(state)
